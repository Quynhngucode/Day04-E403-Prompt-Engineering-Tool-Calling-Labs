...
from src.prompt.system_prompt import build_system_prompt as build_order_system_prompt
...
def build_system_prompt(today: str | None = None) -> str:
    ...
    """
    return build_order_system_prompt(today=today)
...
def extract_saved_order(tool_calls: list[ToolCallRecord]) -> tuple[dict | None, str | None]:
    ...
    for record in reversed(tool_calls):
        ...
        if not payload.get("ok"):
            return None, None
        return payload.get("order"), payload.get("path")
