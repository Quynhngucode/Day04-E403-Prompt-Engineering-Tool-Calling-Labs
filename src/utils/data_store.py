...
class OrderDataStore:
    """
    Local catalog/order store used by the LangChain tools.
    """
    ...
    def _normalize_items(self, items: list[OrderLineInput | dict]) -> list[OrderLineInput]:
            ...
            results.append(
                (
                    ...
                    product.product_id,
                    {
                        ...
                        "next_step": "Call get_product_details with chosen product IDs.",
                    },
                )
            )
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        normalized_tier = (customer_tier or "standard").strip().lower()
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        ...
        if discount_rate not in {0.1, 0.2}:
            return {"status": "error", "ok": False, "errors": [f"Unsupported discount rate: {discount_rate}."]}
        ...
        if errors:
            return {"status": "error", "ok": False, "errors": errors, "items": lines}
